...
fn test_open_credit_line_below_min_fails() {
    ...
    assert!(result.is_err(), "Expected error when opening line below min");
    let err = result.err().unwrap();
    assert_eq!(
        err.unwrap(),
        ContractError::LimitOutOfBounds,
...
fn test_open_credit_line_above_max_fails() {
    ...
    assert!(result.is_err(), "Expected error when opening line above max");
    let err = result.err().unwrap();
    assert_eq!(
        err.unwrap(),
        ContractError::LimitOutOfBounds,
...
fn test_update_risk_params_increase_above_max_fails() {
    ...
    assert!(result.is_err(), "Expected error when increasing limit above max");
    let err = result.err().unwrap();
    assert_eq!(
        err.unwrap(),
        ContractError::LimitOutOfBounds,
...
fn test_update_risk_params_decrease_below_min_fails() {
    ...
    assert!(result.is_err(), "Expected error when decreasing limit below min");
    let err = result.err().unwrap();
    assert_eq!(
        err.unwrap(),
        ContractError::LimitOutOfBounds,
