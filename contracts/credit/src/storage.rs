...
pub enum DataKey {
    ...
    TreasuryBalance,
    /// Address of the collateral token contract.
    CollateralToken,
...
/// Get the collateral token address, if configured.
pub fn get_collateral_token(env: &Env) -> Option<Address> {
    env.storage().instance().get(&DataKey::CollateralToken)
}

/// Set the collateral token address (admin only, enforced by caller).
...
/// Get the per-borrower collateral balance.
...
/// Set the per-borrower collateral balance.
...
/// Get the minimum collateral ratio in basis points.
...
/// Set the treasury address (admin only, enforced by caller).
...
/// Add to the accumulated treasury balance.
pub fn add_treasury_balance(env: &Env, amount: i128) {
    let current: i128 = get_treasury_balance(env);
    env.storage()
        .instance()
        .set(&DataKey::TreasuryBalance, &current.saturating_add(amount));
}

/// Clear the accumulated treasury balance.
pub fn clear_treasury_balance(env: &Env) {
    env.storage().instance().remove(&DataKey::TreasuryBalance);
}
