...
/// Set or clear a per-borrower max utilization ratio cap in basis points.
...
/// Remove a borrower from the draw blocklist.
...
/// Get the configured treasury withdrawal address, if any.
pub fn get_treasury_address(env: &Env) -> Option<Address> {
    env.storage().instance().get(&DataKey::TreasuryAddress)
}

/// Set the treasury withdrawal address.
...
/// Get the accumulated treasury accounting balance.
...
/// Increase the treasury accounting balance using checked arithmetic.
pub fn add_treasury_balance(env: &Env, amount: i128) -> i128 {
    if amount < 0 {
...
/// Clear the treasury accounting balance after a successful withdrawal.
...
/// Return the collateral token. The current implementation reuses the liquidity token.
pub fn get_collateral_token(env: &Env) -> Option<Address> {
    env.storage().instance().get(&DataKey::LiquidityToken)
}

/// Get a borrower's collateral balance.
...
/// Set a borrower's collateral balance.
...
/// Get the minimum collateral ratio in basis points, if configured.
