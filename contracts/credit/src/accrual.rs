...
/// Materialize lazy interest accrual for a bounded set of borrowers.
///
/// Missing lines and non-active lines are skipped so keepers can submit mixed
/// batches without reverting the entire call. Each successful write uses the
/// standard `persist_credit_line` path, preserving the global utilization
/// accumulator and TTL behavior.
pub fn accrue_batch(env: &Env, borrowers: Vec<Address>) {
    for borrower in borrowers.iter() {
        let Some(stored_line) = crate::storage::get_credit_line(env, &borrower) else {
            continue;
        };
